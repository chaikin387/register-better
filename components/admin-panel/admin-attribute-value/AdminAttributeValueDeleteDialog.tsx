...
import { deleteAdminAttributeValue } from '@/app/(admin)/_actions/attribute/delete-attribute-value'
...
export function AdminAttributeValueDeleteDialog({
  isOpen,
  value,
  onClose,
}: Props) {
  ...
  function handleConfirm(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    startTransition(async () => {
      const result = await deleteAdminAttributeValue(
        value.id,
        value.attributeId
      )
    ...
    >
      <AlertDialogContent size='sm'>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Вы собираетесь удалить значение{' '}
            <span className='font-extrabold'>"{value.value}".</span> Это
            действие необратимо.
          </AlertDialogDescription>
        </AlertDialogHeader>
